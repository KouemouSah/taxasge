/**
 * Dynamic Workflow Action Page - CNEDOGE Residencia
 * Displays service requests table with filters for pending, validation, appointments, history
 *
 * @route /[locale]/dashboard/agent/cnedoge-residencia/[workflowGroup]/[action]
 * @date 2026-01-25
 */
...
// Residencia workflow codes for filter
const RESIDENCIA_WORKFLOWS = [
...
export default function WorkflowActionPage() {
  ...
  const {
    ...
    workflowCode: workflowCode !== 'all' ? workflowCode : undefined,
  ...
  const handleRowClick = (requestId: string) => {
    router.push(`/${locale}/dashboard/agent/cnedoge-residencia/request/${requestId}`);
  };
  ...
  const getWorkflowLabel = (wfCode: string) => {
  ...
  if (!isStandardAction) {
    return (
      <GenericFilteredList
        entityCode={ENTITY_CODE}
        entityUrlCode="cnedoge-residencia"
        action={action}
    ...
    <div className="space-y-6">
                ...
                <TableBody>
                  {requests.map((request) => (
                    ...
                    >
                      ...
                      <TableCell>
                        {getWorkflowLabel(request.workflowCode)}
                      </TableCell>
                      <TableCell>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {STANDARD_ACTIONS.map((actionItem) => (
          <Link
            key={actionItem}
            href={`/${locale}/dashboard/agent/cnedoge-residencia/${workflowGroup}/${actionItem}`}
          >
