...
class WizardSessionService:
    ...
    async def _persist_session_data(
        ...
        # Priority: appointment_data > form_data > fallback to main office
        ...
        if not entity_location_id:
            form_loc = session.get("form_data", {}).get("entity_location_id")
            if form_loc:
                try:
                    entity_location_id = UUID(form_loc)
                except (ValueError, TypeError):
                    pass
        if not entity_location_id:
            ...
            if workflow:
                ...
                main_office = await db.fetchrow(
                    "SELECT id FROM entity_locations "
                    "WHERE entity_code = $1 AND is_main_office = TRUE AND is_active = TRUE "
                    "LIMIT 1",
                    entity_code,
                )
                if main_office:
                    entity_location_id = main_office["id"]
