...
class WorkflowMappingRepository:
    ...
    async def find_mapping_for_workflow(
        self,
        workflow_code: str
    ) -> Optional[Dict[str, Any]]:
        """
        Find the matching mapping rule for a workflow code

        Args:
            workflow_code: Workflow code (e.g., 'PASAPORTE_NUEVO')

        Returns:
            Matching mapping dict or None
        """
        # Get the category prefix from the workflow code
        category = workflow_code.split('_')[0] if '_' in workflow_code else workflow_code
        ...
        result = await self.db.fetchrow("""
            ...
            FROM workflow_menu_mapping
            WHERE is_active = true
              AND $1 LIKE REPLACE(workflow_pattern, '%', '')
