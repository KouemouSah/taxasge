...
class AuthService:
    ...
    async def register(
        ...
        try:
            ...
            # Send verification email (NON-BLOCKING - allow registration even if email fails)
            try:
                ...
                if email_sent:
                    ...
                    logger.warning(f"Failed to send verification email to {user.email}, but user account created successfully")

            except Exception as email_error:
                # Log error but don't block registration
                logger.error(f"Email sending failed during registration for {user.email}: {str(email_error)}")
                logger.info(f"User {user.email} registered successfully despite email failure. User can request resend via /email/resend")
