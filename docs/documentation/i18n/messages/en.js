...
window.__I18N__.en =
{
  ...
  "aio": {
    ...
    "toc": {
      ...
      "phaseb": "10. Phase B — Backend APM (full instrumentation)"
    },
    "why": {
    ...
    "phaseb": {
      "intro": "Phase A instrumented only the 18 Gemini call sites. Phase B (2026-05-05) extends OTEL coverage to <strong>the entire FastAPI backend</strong> via auto-instrumentation packages — every HTTP request, every <code>asyncpg</code> query, every <code>httpx</code> external call, every Redis op becomes a span. This unlocks Grafana Cloud's <strong>Application Observability</strong> feature (RED metrics + service map auto-derived from Tempo spans).",
      "what": "What gets instrumented",
      "col_layer": "Layer", "col_pkg": "OTEL package", "col_creates": "Spans created",
      "row_fastapi": "Parent span per HTTP request (method, path, status, duration). Excludes /healthz, /metrics, /static/* to keep volume sane.",
      "row_asyncpg": "Span per query (statement, parameters redacted, duration). Identifies slow queries and lock contention.",
      "row_httpx": "Span per outgoing call to Vertex AI / BANGE / Firebase / Grafana API. URL + status + latency.",
      "row_redis": "Span per cache op (GET, SET, DEL, etc.). Identifies cold cache or hot keys.",
      "sampling": "Sampling policy: 100% (zero sampling)",
      "sampling_body": "User decision 2026-05-05: <strong>no sampling</strong> — instrument everything. Trade-off: full visibility on 100+ concurrent agent traces and bundle workflow debug, at the cost of higher Tempo ingestion (50GB/month free tier). If quota saturates, drop to 25% head-sampling on HTTP via <code>TraceIdRatioBased</code> sampler in <code>main.py</code>.",
      "activate": "Activate Application Observability in Grafana",
      "activate_body": "After the next deploy emits the first instrumented spans (~5 min):",
      "act1": "Open <a href=\"https://kouemousah.grafana.net/a/grafana-app-observability-app/landing\" target=\"_blank\" rel=\"noopener\">grafana-app-observability-app/landing</a>",
      "act2": "Click <strong>Activate Application Observability</strong>",
      "act3": "Select the Tempo data source (default: <code>grafanacloud-kouemousah-traces</code>)",
      "act4": "Wait 5-10 min for the auto-derivation pipeline to bootstrap. Service map appears under Observability &gt; Application.",
      "expected": "Expected service map",
      ...
      "cap_services": "<strong>Figure 1 — Services list</strong> (<em>Observability → Application → Services</em>).<br><strong>What it is:</strong> the catalog of all services auto-discovered by Grafana from OTEL spans emitted by the Phase B instrumentation. No manual registration — a service appears as soon as it emits its first span.<br><strong>What you read here:</strong> 3 services found — <code>facil-backend</code> (the FastAPI process), <code>postgres</code> (every asyncpg query becomes a span), <code>redis</code> (every cache get/set). Each row shows <em>Duration p95</em> (95% of requests served under this latency), <em>Errors</em> (% failed), and <em>Rate</em> (req/sec).<br><strong>What the numbers mean:</strong> <code>facil-backend</code> p95 = <strong>487.5 ms</strong> = overall API latency at the slow end. <code>postgres</code> p95 = <strong>725 ms</strong> at rate 0.12 r/s = very few queries, some slow (cron MV refresh). <code>redis</code> p95 = <strong>2.35 s</strong> = high but on rare operations (boot warmup). 0% errors everywhere = clean stack.<br><strong>How to use it:</strong> entry point of any debugging session. If a user reports slowness, start here, sort by p95, drill into the slowest service.",
      "cap_overview": "<strong>Figure 2 — facil-backend Overview</strong>.<br><strong>What it is:</strong> the RED Method (Rate / Errors / Duration) view of one specific service, auto-derived from its spans. The standard SRE dashboard for any backend service.<br><strong>What you read here:</strong> <em>Top row</em> = the 3 RED metrics over the time window. <em>Operations</em> sub-panel = breakdown by HTTP route (<code>GET /health</code> dominates here because it's polled by Cloud Run health-check). <em>Outbound &amp; databases</em> sub-panel = downstream dependencies the service calls.<br><strong>What the numbers mean:</strong> p95 = <strong>487.5 ms</strong> on <code>GET /health</code> is unexpected — health-check should be 5–10 ms. Cause: at boot Cloud Run probes during cold-start when the asyncpg pool is still warming. Solution: add a startup probe distinct from liveness probe. Redis p95 <strong>2.35 s</strong> + Postgres p95 <strong>487 ms</strong> shown in <em>Outbound</em> = blocking calls in the request path; investigate if they are in the hot path.<br><strong>How to use it:</strong> after Figure 1, this is the second screen. The 3 RED metrics tell you <em>where</em> the slowness comes from (route or downstream).",
      "cap_servicemap": "<strong>Figure 3 — Service map</strong>.<br><strong>What it is:</strong> the call graph of the service auto-built from instrumenter spans (<code>asyncpg</code>, <code>redis</code>, <code>httpx</code>). Each node = a service, each edge = N requests/sec with average latency.<br><strong>What you read here:</strong> <code>facil-backend</code> calls 2 dependencies — <code>postgres</code> (211.88 ms/req @ 0.92 r/s = the heart of business traffic, every endpoint reads/writes BD) and <code>redis</code> (129.76 ms/req @ 0.03 r/s = much rarer, mainly cache invalidations). <em>Why doesn't Vertex AI / Firebase / BANGE appear?</em> These are <code>httpx</code> calls that fire only when a user triggers a chatbot, signs a document or pays — low frequency, no traffic during the window captured.<br><strong>What the numbers mean:</strong> 211.88 ms <em>per Postgres request</em> averaged is high (target &lt; 50 ms). Corroborates Figure 2 finding. <em>r/sec</em> = throughput. <em>Avg / p50 / p95</em> on each edge = same RED method applied to the relationship between 2 services.<br><strong>How to use it:</strong> map the architecture in 5 seconds. To detect a critical missing edge, compare with the production architecture diagram. To detect contention, look at the thickest edges (high r/sec).",
      "cap_traces": "<strong>Figure 4 — Traces tab</strong> (TraceQL filter <code>resource.service.name=\"facil-backend\"</code>).<br><strong>What it is:</strong> the list of recent <em>distributed traces</em> (an end-to-end request crossing N spans). One trace = one HTTP request followed across all internal hops.<br><strong>What you read here:</strong> 20 most recent traces in the captured window. Each row = <em>Trace ID</em> (clickable), <em>Trace name</em> (the root span = the operation that started everything), <em>Span count</em> (how many internal hops), <em>Duration</em> (total end-to-end). Mostly <code>SELECT</code> queries (101–906 ms range), one <code>INSERT</code> (981 ms), one <code>REFRESH MATERIALIZED VIEW</code> (599 ms — the <code>request-telemetry-cleanup</code> cron MV refresh from Phase C.2).<br><strong>What the numbers mean:</strong> <em>Span count</em> &gt; 5 = a request that triggered multiple BD calls (sign of N+1 query candidate). <em>Duration</em> at the high end (981 ms INSERT) often reveals a slow SQL statement. The one-off REFRESH is normal — backend cron, runs every 6h.<br><strong>How to use it:</strong> click a trace_id to open the flame graph view = full breakdown FastAPI route → asyncpg query → result. Indispensable for diagnosing complex bottlenecks (e.g. 5-entity bundle workflow taking 4 s — the trace shows exactly which entity locked first)."
    }
  },
