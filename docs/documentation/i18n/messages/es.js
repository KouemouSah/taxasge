...
window.__I18N__.es =
{
  "common": {
    "sidebar": {
      ...
      "link": {
        ...
        "mobile_publishing": "Publicación móvil",
  ...
  "indexpage": {
    ...
    "docmap": {
      ...
      "mobile_publishing": "— Flujo completo de Play Store (EAS Cloud + GitHub Actions + Play Console), validación de desarrollador Android (token ADI), gestión de keystore, resolución de problemas"
    },
    "overview": {
    ...
    "cards": {
      ...
      "mobile_publishing": "Flujo de release de Play Store de extremo a extremo: builds de EAS Cloud, orquestación de GitHub Actions, validación de desarrollador Android (token ADI), gestión de keystore con firma gestionada por EAS y resolución de problemas."
    },
    "dec": {
  ...
  "aio": {
    "html_title": "Observabilidad IA - Documentación Facil",
    "title": "Observabilidad IA — seguimiento de coste y latencia Gemini",
    "description": "Telemetría por llamada en cada petición Gemini / Vertex AI hecha por el backend Facil. 18 puntos de llamada instrumentados en 16 funcionalidades (RAG chatbot, OCR, clasificación, enriquecimiento, enrutamiento, briefing, etc.). Salidas: 1 panel Grafana con 12 paneles + 3 reglas de alerta + traza persistida en BD (tabla <code>ai_call_metrics</code>, mig 325). Privacy by design: nunca se almacena contenido bruto de prompt, solo hashes SHA-256 truncados.",
    "toc": {
      "why": "1. Por qué & análisis de gap",
      "stack": "2. Stack & doble escritura",
      "schema": "3. Esquema BD (mig 325)",
      "wrapper": "4. API Wrapper (traced_generate_sync)",
      "features": "5. Etiquetas feature (16 mappings)",
      "dashboard": "6. Panel (12 paneles)",
      "alerts": "7. Alertas (3 reglas + runbooks)",
      "privacy": "8. Privacidad & seguridad",
      "cohabitation": "9. Convivencia con VertexAIManager",
      "phaseb": "10. Phase B — APM Backend (instrumentación completa)"
    },
    "why": {
      "intro": "Antes de este trabajo, el backend hacía <strong>~17 puntos de llamada Gemini distintos</strong> con <strong>cero observabilidad</strong>. No podíamos responder a 4 preguntas operacionales críticas:",
      "q1": "<strong>Coste</strong>: ¿cuánto gastamos en tokens por día, por modelo, por feature?",
      "q2": "<strong>Latencia</strong>: ¿cuál es el p95/p99 por tipo de llamada (RAG vs OCR vs clasificación)?",
      "q3": "<strong>Fiabilidad</strong>: ¿cuál es el failure rate y de qué tipo (rate-limit / timeout / JSON parse / contenido bloqueado)?",
      "q4": "<strong>Optimización</strong>: ¿qué prompts cuestan más? ¿Hay quick wins?"
    },
    "stack": {
      "intro": "Cada llamada pasa por un wrapper único <code>traced_generate_sync()</code> que emite <strong>tanto</strong> un span OTEL (a Grafana Tempo para el drill-down de traces) <strong>como</strong> una fila en la tabla BD <code>ai_call_metrics</code>.",
      "step1": "Punto de llamada", "step1_sub": "ej: chatbot RAG",
      "step2": "traced_generate_sync", "step2_sub": "app/core/ai_telemetry.py",
      "step3": "model.generate_content()", "step3_sub": "Vertex AI sync vía run_in_executor",
      "fanA": "A. Span OTEL", "fanA_sub": "→ Grafana Tempo (retención 14d)",
      "fanB": "B. asyncio.create_task INSERT", "fanB_sub": "→ ai_call_metrics (BD persistente)",
      "why_double": "Tempo Free tier tiene retención de 14 días — insuficiente para informes mensuales de coste. La BD es la <strong>fuente de verdad a largo plazo</strong>; Tempo hace drill-down debug. Ambas escrituras son <strong>fire-and-forget</strong> vía <code>asyncio.create_task</code> — nunca bloquean la llamada del usuario."
    },
    "schema": {
      "intro": "Tabla <code>ai_call_metrics</code> (20 columnas, 8 índices, 9 CHECK constraints):",
      "col_name": "Columna", "col_type": "Tipo", "col_purpose": "Función",
      "row_id": "Clave primaria + tiempo de ingesta",
      "row_trace": "Correlación OTEL con Tempo",
      "row_model": "gemini | vertex_embedding × chat | embeddings | completion",
      "row_feature": "Contexto Facil (chatbot_rag / ocr / etc.)",
      "row_tokens": "Contadores de uso; total_tokens auto-calculado",
      "row_cost": "Coste FCFA estimado (input × pricing.input + output × pricing.output)",
      "row_status": "ms end-start + enum 6 estados (success / error / rate_limited / timeout / content_blocked / json_parse_error)",
      "row_hash": "SHA-256 truncado — identificador privacy-safe (sin mapping inverso)",
      "views": "Dos vistas de agregación alimentan los paneles: <code>v_ai_cost_daily</code> (90 días) y <code>v_ai_cost_hourly</code> (7 días). Ambas grant a <code>looker_readonly</code>."
    },
    "wrapper": {
      "intro": "El módulo <code>app/core/ai_telemetry.py</code> expone 4 wrappers:",
      "col_when": "Cuándo usar", "col_signature": "Firma",
      "row_sync": "<code>model.generate_content()</code> sync + <code>run_in_executor</code>. <strong>Patrón más común en este codebase.</strong>",
      "row_async": "<code>generate_content_async()</code> async si el SDK lo soporta",
      "row_emb_sync": "<code>model.get_embeddings()</code> sync",
      "row_emb_async": "Embeddings async",
      "principles": "Principios: (1) <strong>import OTEL soft</strong> — el módulo carga sin opentelemetry-sdk; los spans se vuelven no-op; persist BD continúa. (2) <strong>persist BD nunca bloquea</strong> — <code>asyncio.create_task</code> + try/except. (3) <strong>privacy by construction</strong> — el contenido del prompt nunca se almacena, solo el hash 16-char SHA-256."
    },
    "features": {
      "intro": "Cada punto de llamada se etiqueta con una string <code>feature</code> usada para la segmentación coste/latencia en el panel:",
      "col_label": "Feature", "col_module": "Módulo", "col_volume": "Volumen",
      "vol_high": "Alto", "vol_vhigh": "Muy alto", "vol_med": "Medio", "vol_low": "Bajo (cron)"
    },
    "dashboard": {
      "intro": "UID: <code>facil-ai-observability</code>. <strong>12 paneles</strong> en 5 secciones de filas. Listado en <code>/admin/dashboards</code> bajo categoría «security» (rls_mode <code>admin_only</code>).",
      "s1": "<strong>💰 Coste</strong>: hoy XAF, MTD XAF, total llamadas hoy, tokens in/out hoy (4 stats)",
    ...
    "alerts": {
      ...
      "runbook_cost": "Runbook — Pico de coste",
      "runbook_error": "Runbook — Pico de tasa de error",
      "runbook_latency": "Runbook — Degradación p95 latencia",
      "cost_body": "1. Abrir el panel, drill en «Top features por coste (7d)». 2. Identificar la feature dominante. 3. Revisar «Top prompt hashes por coste (7d)» por prompts costosos duplicados (candidato a caching). 4. Si routing/briefing/enrichment cron es la causa, throttler la frecuencia. 5. Si chatbot_rag explota, sospechar un bot o agente en bucle — revisar audit_logs.",
      "error_body": "1. Abrir «Errors by status type (7d)». 2. <strong>rate_limited</strong> → cuota Vertex AI alcanzada, aumentar via Google Cloud Console. 3. <strong>timeout</strong> → verificar el estado Gemini; considerar asyncio.wait_for más largo. 4. <strong>content_blocked</strong> → filtro SAFETY activado, revisar la plantilla del prompt. 5. <strong>json_parse_error</strong> → regla memoria #21: asegurar <code>response_mime_type=\"application/json\"</code> en GenerationConfig.",
      "latency_body": "1. Revisar el panel «p95 latency por feature». 2. Si <code>ocr</code> pica, sospechar de PDFs grandes. 3. Si <code>chatbot_rag</code> pica, revisar la latencia de búsqueda pgvector. 4. Si todas las features pican juntas, sospechar una degradación regional de Vertex AI."
    },
    "privacy": {
      "no_content": "<strong>Sin contenido bruto de prompt</strong> almacenado. Solo el hash SHA-256 truncado a 16 chars. Sin tabla de mapping inverso. RGPD-safe by design.",
      "token_hash": "<strong>Formato prompt_hash</strong> enforced por CHECK BD <code>chk_aim_prompt_hash_format</code>: <code>^[a-f0-9]{16}$</code>",
      "user_fk": "<strong>user_id FK ON DELETE SET NULL</strong> — el borrado de un user limpia el enlace sin borrar la fila de auditoría",
      "token_secret": "<strong>Token OTLP via Secret Manager</strong> (<code>grafana-otlp-token</code>) — CAP token scopeado a <code>traces:write</code>",
      "iam": "<strong>RLS</strong>: dashboard <code>rls_mode='admin_only'</code> — los no-admins no ven datos de coste ni siquiera vía <code>/api/v1/dashboards/reports-config</code>"
    },
    "cohabitation": {
      "intro": "El singleton existente <code>app/modules/shared/services/vertex_ai_manager.py</code> está INTENCIONALMENTE preservado. Los dos sistemas son complementarios, no redundantes:",
      "col_concern": "Preocupación",
      "row_circuit": "Circuit breaker (10 fail/60s cooldown)",
      "row_realtime": "Lectura stats in-memory sub-μs",
      "row_persist": "Persistencia BD (cross-worker, sobrevive reinicio)",
      "row_cost": "Coste en XAF",
      "row_tags": "Tags por feature/modelo/user/trace",
      "row_status": "Enum status (6 valores)",
      "pattern": "Patrón de llamada: <code>await traced_generate_sync(...)</code> seguido de <code>VertexAIManager().track_usage(response, \"X\")</code> + <code>track_success()</code>. Ambos coexisten; ninguno bloquea al otro."
    },
    "phaseb": {
      "intro": "La fase A instrumentó solo los 18 puntos de llamada Gemini. La fase B (2026-05-05) extiende la cobertura OTEL a <strong>todo el backend FastAPI</strong> vía auto-instrumentación — cada petición HTTP, cada query <code>asyncpg</code>, cada llamada <code>httpx</code> externa, cada op Redis se convierte en un span. Esto desbloquea la feature <strong>Application Observability</strong> de Grafana Cloud (RED metrics + service map auto-derivados de los spans Tempo).",
      "what": "Qué se instrumenta",
      "col_layer": "Capa", "col_pkg": "Paquete OTEL", "col_creates": "Spans creados",
      "row_fastapi": "Span padre por petición HTTP (método, path, status, duración). Excluye /healthz, /metrics, /static/* para mantener el volumen razonable.",
      "row_asyncpg": "Span por query (statement, parámetros redactados, duración). Identifica queries lentas y lock contention.",
      "row_httpx": "Span por llamada saliente a Vertex AI / BANGE / Firebase / Grafana API. URL + status + latencia.",
      "row_redis": "Span por op de caché (GET, SET, DEL, etc.). Identifica cold cache o hot keys.",
      "sampling": "Política de sampling: 100% (sin sampling)",
      "sampling_body": "Decisión usuario 2026-05-05: <strong>sin sampling</strong> — instrumentar todo. Trade-off: visibilidad completa sobre 100+ trazas de agentes concurrentes y debug bundle workflow, a costa de mayor ingesta Tempo (50GB/mes free tier). Si la cuota se satura, bajar a 25% head-sampling sobre HTTP via <code>TraceIdRatioBased</code> en <code>main.py</code>.",
      "activate": "Activar Application Observability en Grafana",
      "activate_body": "Después de que el próximo deploy emita los primeros spans instrumentados (~5 min):",
      "act1": "Abrir <a href=\"https://kouemousah.grafana.net/a/grafana-app-observability-app/landing\" target=\"_blank\" rel=\"noopener\">grafana-app-observability-app/landing</a>",
      "act2": "Hacer clic en <strong>Activate Application Observability</strong>",
      "act3": "Seleccionar la fuente de datos Tempo (por defecto: <code>grafanacloud-kouemousah-traces</code>)",
      "act4": "Esperar 5-10 min para que el pipeline de auto-derivación arranque. El service map aparece bajo Observability &gt; Application.",
      "expected": "Service map esperado",
      "benefits": "Beneficios concretos",
      "b1": "<strong>Debug bundle workflow</strong> — trazar un solo <code>service_request_id</code> a través de web → backend → 5 locks de entidades → BANGE → email → bóveda, end-to-end en un clic",
      "b2": "<strong>Lock contention BD</strong> — heatmap de queries asyncpg lentas expone deadlocks antes de que los usuarios lo noten",
      "b3": "<strong>Regresión de despliegue</strong> — p95 latencia por endpoint pre/post deploy; decisión de revert en 30s",
      "b4": "<strong>Detección de anomalías auto</strong> — el Knowledge Graph Grafana correlaciona pico + deploy + query lenta sin investigación manual",
      "cap_services": "<strong>Figura 1 — Lista de servicios</strong> (<em>Observability → Application → Services</em>).<br><strong>Qué es:</strong> el catálogo de todos los servicios auto-descubiertos por Grafana a partir de los spans OTEL emitidos por la instrumentación Phase B. Sin registro manual — un servicio aparece en cuanto emite su primer span.<br><strong>Qué se lee aquí:</strong> 3 servicios encontrados — <code>facil-backend</code> (el proceso FastAPI), <code>postgres</code> (cada query asyncpg se convierte en span), <code>redis</code> (cada get/set del cache). Cada fila muestra <em>Duration p95</em> (95% de las requests servidas bajo esta latencia), <em>Errors</em> (% fallidas), y <em>Rate</em> (req/seg).<br><strong>Qué significan los números:</strong> <code>facil-backend</code> p95 = <strong>487,5 ms</strong> = latencia API global en el peor 5%. <code>postgres</code> p95 = <strong>725 ms</strong> a rate 0,12 r/s = pocas queries, algunas lentas (cron MV refresh). <code>redis</code> p95 = <strong>2,35 s</strong> = alto pero en operaciones raras (warmup en boot). 0% errores en todas partes = stack limpio.<br><strong>Cómo usarlo:</strong> punto de entrada de cualquier sesión de debug. Si un usuario reporta lentitud, empezamos aquí, ordenamos por p95, y hacemos drill en el servicio más lento.",
      "cap_overview": "<strong>Figura 2 — Vista general facil-backend</strong>.<br><strong>Qué es:</strong> la vista RED Method (Rate / Errors / Duration) de un servicio específico, auto-derivada de los spans. El dashboard SRE estándar para cualquier servicio backend.<br><strong>Qué se lee aquí:</strong> <em>Fila superior</em> = las 3 métricas RED sobre la ventana temporal. Sub-panel <em>Operations</em> = desglose por ruta HTTP (aquí <code>GET /health</code> domina porque lo poll el health-check de Cloud Run). Sub-panel <em>Outbound &amp; databases</em> = las dependencias downstream que el servicio llama.<br><strong>Qué significan los números:</strong> p95 = <strong>487,5 ms</strong> en <code>GET /health</code> es inesperado — health-check debería ser 5-10 ms. Causa: en boot, Cloud Run lanza probes durante el cold-start mientras el pool asyncpg se inicializa. Solución: añadir un startup probe distinto del liveness probe. Redis p95 <strong>2,35 s</strong> + Postgres p95 <strong>487 ms</strong> visibles en <em>Outbound</em> = llamadas bloqueantes en el request path; investigar si están en el hot path.<br><strong>Cómo usarlo:</strong> tras la Figura 1, esta es la 2ª pantalla. Las 3 métricas RED dicen <em>de dónde</em> viene la lentitud (ruta o dependencia).",
      "cap_servicemap": "<strong>Figura 3 — Service map</strong>.<br><strong>Qué es:</strong> el grafo de llamadas del servicio auto-construido desde los spans de los instrumenters (<code>asyncpg</code>, <code>redis</code>, <code>httpx</code>). Cada nodo = un servicio, cada arista = N requests/seg con latencia media.<br><strong>Qué se lee aquí:</strong> <code>facil-backend</code> llama a 2 dependencias — <code>postgres</code> (211,88 ms/req @ 0,92 r/s = el corazón del tráfico de negocio, todos los endpoints leen/escriben BD) y <code>redis</code> (129,76 ms/req @ 0,03 r/s = mucho más raro, sobre todo invalidaciones de cache). <em>¿Por qué no aparecen Vertex AI / Firebase / BANGE?</em> Son llamadas <code>httpx</code> que solo se disparan cuando un usuario lanza un chatbot, firma un documento o paga — frecuencia baja, sin tráfico durante la ventana capturada.<br><strong>Qué significan los números:</strong> 211,88 ms <em>por request Postgres</em> en promedio es alto (target &lt; 50 ms). Corrobora el hallazgo de la Figura 2. <em>r/sec</em> = throughput. <em>Avg / p50 / p95</em> en cada arista = mismo método RED aplicado a la relación entre 2 servicios.<br><strong>Cómo usarlo:</strong> mapear la arquitectura en 5 segundos. Para detectar un edge crítico ausente, comparar con el diagrama de arquitectura producción. Para detectar contención, mirar las aristas más gruesas (rate alto).",
      "cap_traces": "<strong>Figura 4 — Pestaña Traces</strong> (filtro TraceQL <code>resource.service.name=\"facil-backend\"</code>).<br><strong>Qué es:</strong> la lista de <em>distributed traces</em> recientes (una trace = una request end-to-end atravesando N spans). Una trace = una request HTTP seguida a través de todos los hops internos.<br><strong>Qué se lee aquí:</strong> 20 trazas más recientes en la ventana capturada. Cada fila = <em>Trace ID</em> (clicable), <em>Trace name</em> (el span raíz = la operación que disparó todo), <em>Span count</em> (cuántos hops internos), <em>Duration</em> (total end-to-end). Principalmente queries <code>SELECT</code> (101–906 ms), un <code>INSERT</code> (981 ms), un <code>REFRESH MATERIALIZED VIEW</code> (599 ms — el cron <code>request-telemetry-cleanup</code> refrescando la MV en Phase C.2).<br><strong>Qué significan los números:</strong> <em>Span count</em> &gt; 5 = una request que disparó múltiples BD calls (señal potencial de N+1 query). <em>Duration</em> en el rango alto (981 ms INSERT) suele revelar una instrucción SQL lenta. El REFRESH puntual es normal — cron backend, corre cada 6h.<br><strong>Cómo usarlo:</strong> clic en un trace_id abre la vista flame graph = desglose completo ruta FastAPI → query asyncpg → resultado. Indispensable para diagnosticar bottlenecks complejos (ej: un workflow bundle 5-entidades tomando 4 s — la trace muestra exactamente qué entidad lockeó primero)."
    }
  },
