/* Facil Docs - SVG Icons (Lucide-style, MIT-licensed paths)
   Inline SVG for offline strict. No CDN. No dependency.
   Usage: window.ICONS.user → <svg ...>...</svg> string
   Or window.icon('user', { size: 24, className: 'extra-class' })
*/

(function () {
  'use strict';

  // Raw SVG path data (24x24 viewbox, stroke 2, Lucide-derived)
  var PATHS = {
    // Personas
  ...
  var PERSONA_ICONS = {
    ...
    dgi_agent: 'shield-check',
    ministry_agent: 'landmark',
    supervisor: 'users',
    admin: 'settings',
    transverse: 'globe'
  };
  ...
  // Build SVG element from path data
  window.icon = function (name, opts) {
    ...
    return '<svg xmlns="http://www.w3.org/2000/svg" width="' + size + '" height="' + size +
      ...
      '" stroke-linecap="round" stroke-linejoin="round" class="icon ' + className + '" aria-hidden="true">' +
      paths + '</svg>';
  };

  window.iconForPersona = function (personaId, opts) {
    return window.icon(PERSONA_ICONS[personaId] || 'user', opts);
  };

  window.iconForStatus = function (status, opts) {
    return window.icon(STATUS_ICONS[status] || 'circle-dashed', opts);
  };

  window.ICONS = PATHS;
  window.PERSONA_ICON_MAP = PERSONA_ICONS;
  window.STATUS_ICON_MAP = STATUS_ICONS;
})();
